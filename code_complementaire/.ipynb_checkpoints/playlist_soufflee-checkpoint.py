...
def recommandation_souflee(playlist_id, audiofeatures,sp):
    ...
    playlist_cible = pd.read_csv('data/data_o.csv')
    ...
    for i in range (len(source)) : #pour chaque morceau de la playlist initiale
        ...
        for audio in audiofeatures:#On boucle sur les audiofeatures pour les ajouter à playlist
            playlist[i].append(source[audio][i])
        
        #on enleve nos morceaux de la playlist de la base de données, pour ne pas que nos propres morceaux soient reproposés
